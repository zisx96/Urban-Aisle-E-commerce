...
const NumberInput = ({max=1,min=1,quantity, onChangeQuantity}) => {

    const [value, setValue] = useState(quantity ?? 1);
    const [message,setMessage] = useState('');

    const displayMaxStock = useCallback(() => {
        setMessage(`Not Enough Quantity`);
    ...
    const displayMinQuantity = useCallback(() => {
        setMessage(`At least ${min} item should be present`);
  ...
  return (
    ...
    {message && <p className='text-sm text-center p-2 text-red-500 '>{message}</p>}
    </>
  )
}
